        ...
        .then(response => {
            response.json()
                .then(snaps => {
                    ...
                    .bindPopup( `<div><p>Snap aghjunghjatu u ${new Date(t.date) }</p></div><video style="object-fit: fill;" controls><source src="/resources/${t.resource}" type="video/mp4">Your browser does not support the video tag.</video>` )
                    .addTo( map ));
